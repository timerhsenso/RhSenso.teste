...
namespace RhS.Api.Controllers.SEG
{
    ...
    public class SistemasController : ControllerBase
    {
        ...
        public async Task<IActionResult> Create([FromBody] SistemaCreateDto dto, CancellationToken ct)
        {
            try
            {
                var codigo = await _service.CreateAsync(dto, ct);
                return CreatedAtAction(nameof(GetById), new { codigo }, null);
            }
            catch (ArgumentException ex)
            {
                _logger.LogWarning(ex, "Payload inválido");
                return ValidationProblem(title: "Dados inválidos", detail: ex.Message, statusCode: 400);
            }
            catch (InvalidOperationException ex)
            {
                _logger.LogWarning(ex, "Conflito");
