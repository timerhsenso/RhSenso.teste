...
namespace RhS.Web.Services.ApiClients
{
    public class BotoesApi : IBotoesApi
    {
        ...
        {
            var r = await _http.PostAsJsonAsync("api/v1/botoes", dto, _json, ct); 
        ...
        {
            var r = await _http.PutAsJsonAsync($"api/v1/botoes/{sistema}/{funcao}/{nome}", dto, _json, ct); 
            r.EnsureSuccessStatusCode();
        }
        
        public async Task DeleteAsync(string sistema, string funcao, string nome, CancellationToken ct = default)
        {
            var r = await _http.DeleteAsync($"api/v1/botoes/{sistema}/{funcao}/{nome}", ct); 
            r.EnsureSuccessStatusCode();
        }
    }
}